...
-   **Why does the `intl` extension show weird characters that look like diamonds with question marks
    ...
    $result = \WordPressImporter\Rowbot\Idna\Idna::toUnicode($input);
    echo $result->getDomain(); // 憡?Ⴔ.xn--1ug73gl146a
    echo $result->hasError(\WordPressImporter\Rowbot\Idna\Idna::ERROR_DISALLOWED); // true
    ```
...
must first change the value of the `\WordPressImporter\Rowbot\Idna::UNICODE_VERSION` constant, like so:
