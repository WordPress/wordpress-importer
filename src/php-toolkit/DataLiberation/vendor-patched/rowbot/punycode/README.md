...
-   `\WordPressImporter\Rowbot\Punycode\Exception\OutputSizeExceededException` - If the size of the output string
    exceeds the maximum size specified.
-   `\WordPressImporter\Rowbot\Punycode\Exception\OverflowException` - If integer overflow occurs.
-   `\WordPressImporter\Rowbot\Punycode\Exception\InvalidInputException` - If input contains non-ASCII bytes or mapping
...
} catch (\WordPressImporter\Rowbot\Punycode\Exception\PunycodeException $e) {
