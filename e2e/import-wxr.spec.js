...
async function performImport(page, { mapAuthorsToAdmin = false } = {}) {
	...
	const wxrPath = path.resolve(__dirname, '../phpunit/data/wxr-comprehensive.xml');
	const fileInput = page.locator('#upload, input[type="file"][name="import"]');
	await fileInput.waitFor({ state: 'visible' });
	await fileInput.setInputFiles(wxrPath);
...
async function verifyImportedData(page, request, { expectAuthorSlug = 'admin' } = {}) {
	// Verify post
	const postRes = await request.get('/wp-json/wp/v2/posts?_embed=1&search=Comprehensive%20Post&per_page=10');
	...
	const categories = terms.filter((t) => t.taxonomy === 'category').map((t) => (t.slug || '').toLowerCase());
	const tags = terms.filter((t) => t.taxonomy === 'post_tag').map((t) => (t.slug || '').toLowerCase());
	...
	const commentsRes = await request.get(`/wp-json/wp/v2/comments?post=${post.id}`);
	expect(commentsRes.ok()).toBeTruthy();
	const comments = await commentsRes.json();
	expect(comments.some((c) => (c.content?.rendered || '').includes('Great post'))).toBeTruthy();
	
	// Verify page exists
	const pageRes = await request.get('/wp-json/wp/v2/pages?search=Comprehensive%20Page&per_page=10');
